...
class BinanceCredentialsRequest(BaseModel):
    ...
    is_mainnet: bool = Field(default=True, description="True for mainnet, False for testnet")
...
async def save_testnet_credentials(
    ...
    try:
        ...
        if success:
            ...
            return CredentialsResponse(
                success=False,
                message="Failed to save testnet credentials"
            )
    ...
    try:
        ...
        async with binance_service as service:
            test_result = await service.test_connection(
                request.api_key,
                request.secret_key,
                is_testnet=not request.is_mainnet
            )
        ...
        if success:
            ...
            return CredentialsResponse(
                success=False,
                message="Failed to save Binance credentials"
            )
    ...
    try:
        ...
        status = {
            "mainnet": {
                ...
                "created_at": credentials['mainnet']['created_at'] if credentials['mainnet'] else None,
                "updated_at": credentials['mainnet']['updated_at'] if credentials['mainnet'] else None
            },
            "testnet": {
                "configured": has_testnet,
                "created_at": credentials['testnet']['created_at'] if credentials['testnet'] else None,
                "updated_at": credentials['testnet']['updated_at'] if credentials['testnet'] else None
            }
        }
        ...
        preferred_env = "mainnet" if has_mainnet else ("testnet" if has_testnet else None)
    ...
    try:
        async with binance_service as service:
            ...
            if not result['success']:
                ...
                logger.info(f"✅ Connection test succeeded for user {current_user['id']}")
                logger.info(f"   Environment: {result.get('environment')}")
                logger.info(f"   Account info: {result.get('account_info', {}).get('accountType', 'N/A')}")
...
async def validate_saved_credentials(
    ...
    try:
        ...
        if not creds:
            return CredentialsResponse(
                success=False,
                message=f"No {credential_type} credentials found"
            )
...
async def delete_binance_credentials(
    ...
    try:
        ...
        if success:
            env_desc = "all" if is_mainnet is None else ("mainnet" if is_mainnet else "testnet")
            return CredentialsResponse(
                success=True,
