...
async def get_connection_status(
    ...
    try:
        ...
        if binance_creds["testnet"]:
            ...
            try:
                testnet_creds_binance = await credentials_db.get_binance_credentials(
                ...
                if testnet_creds_binance:
                    async with binance_service as service:
                        ...
                        connection_status["binance_testnet"]["connected"] = test_result[
                            "success"
                        ]
                        connection_status["binance_testnet"]["status"] = (
                            "connected" if test_result["success"] else "error"
                        )
                        connection_status["binance_testnet"]["error"] = (
                            test_result.get("error")
                            if not test_result["success"]
                            else None
                        )
                        ...
                        if (
                            test_result["success"]
                            and connection_status["overall_status"] == "disconnected"
                        ):
                            connection_status["overall_status"] = "connected"
        ...
        for cred in testnet_creds:
            if (
                ...
                and not connection_status["binance_testnet"]["configured"]
            ):
                connection_status["binance_testnet"]["configured"] = True
                try:
                    testnet_cred = await credentials_db.get_testnet_credentials(
                        user_id, "binance"
                    )
                    if testnet_cred:
                        async with binance_service as service:
                            ...
                            if (
                                test_result["success"]
                                and connection_status["overall_status"]
                                == "disconnected"
                            ):
                                connection_status["overall_status"] = "connected"
                                connection_status["active_environment"] = "testnet"
