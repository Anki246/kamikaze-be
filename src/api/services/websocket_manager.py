...
class WebSocketManager:
    ...
    async def connect(self, client_id: str, websocket: WebSocket):
        ...
        try:
            ...
            logger.info(f"WebSocket client {client_id} connected. Total connections: {self.connection_count}")
    ...
    async def disconnect(self, client_id: str):
        ...
        try:
            ...
            self.connection_count -= 1
            logger.info(f"WebSocket client {client_id} disconnected. Total connections: {self.connection_count}")
    ...
    async def broadcast_agent_update(self, agent_id: str, event_type: str, data: Optional[Dict[str, Any]] = None, user_id: Optional[int] = None):
        ...
        if subscribed_clients:
            logger.info(f"📡 Broadcasting agent update for {agent_id} to {len(subscribed_clients)} subscribed clients")
            disconnected_clients = []
            ...
            for client_id in subscribed_clients:
                if client_id in self.active_connections:
                    try:
                        ...
                        logger.error(f"❌ Error sending agent update to client {client_id}: {e}")
                        disconnected_clients.append(client_id)
            ...
            logger.debug(f"📡 No clients subscribed to agent {agent_id}, skipping broadcast")
    ...
    async def broadcast_cycle_analysis(self, agent_id: str, cycle_data: Dict[str, Any], user_id: Optional[int] = None):
        ...
        if subscribed_clients:
            logger.info(f"📊 Broadcasting cycle analysis for {agent_id} to {len(subscribed_clients)} subscribed clients")
            disconnected_clients = []
            ...
            for client_id in subscribed_clients:
                if client_id in self.active_connections:
                    try:
                        ...
                        logger.error(f"❌ Error sending cycle analysis to client {client_id}: {e}")
                        disconnected_clients.append(client_id)
            ...
            logger.debug(f"📊 No clients subscribed to agent {agent_id} for cycle analysis")

    async def broadcast_trade_execution(self, agent_id: str, trade_data: Dict[str, Any], user_id: Optional[int] = None):
        ...
        if subscribed_clients:
            logger.info(f"💰 Broadcasting trade execution for {agent_id} to {len(subscribed_clients)} subscribed clients")
            disconnected_clients = []
            ...
            for client_id in subscribed_clients:
                if client_id in self.active_connections:
                    try:
                        ...
                        logger.error(f"❌ Error sending trade execution to client {client_id}: {e}")
                        disconnected_clients.append(client_id)
            ...
            logger.debug(f"💰 No clients subscribed to agent {agent_id} for trade execution")
