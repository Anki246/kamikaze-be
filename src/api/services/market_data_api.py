...
class MarketDataAPI:
    ...
    async def get_ticker_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            result = await self.binance_client.call_tool("get_24h_ticker", {"symbol": symbol})
    ...
    async def get_market_data(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        cache_key = self._get_cache_key("market_data", symbols=",".join(sorted(symbols)))
        cached_data = self._get_cache(cache_key)
        if cached_data:
            return cached_data
    ...
    async def get_market_stats(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not market_data_response.get("success") or not market_data_response.get("data"):
                return {
                    "success": False,
                    "total_volume": 0,
                    "avg_change": 0,
            ...
            top_gainer = max(prices, key=lambda x: x.get("changePercent24h", 0)) if prices else None
            top_loser = min(prices, key=lambda x: x.get("changePercent24h", 0)) if prices else None
    ...
    async def get_technical_indicators(self, symbol: str, timeframe: str = "1h", 
        ...
        cache_key = self._get_cache_key("indicators", symbol=symbol, timeframe=timeframe, 
        ...
        try:
            ...
            result = await self.binance_client.call_tool("calculate_technical_indicators", {
                "symbol": symbol,
                "timeframe": timeframe,
    ...
    async def get_account_balance(self, user_id: int = None) -> Dict[str, Any]:
        ...
        if user_id is None:
            try:
                ...
                if user_context:
                    ...
                    logger.info(f"🔧 Using user from context: {user_id} ({user_context.email})")
                else:
        ...
        try:
            logger.info(f"Getting account balance for user {user_id} (type: {type(user_id)})")
            ...
            if user_id is not None:
                ...
                logger.info(f"🔧 Calling set_user_credentials_tool with user_id={user_id_int}")
                creds_result = await self.binance_client.call_tool("set_user_credentials_tool", {
                    "user_id": user_id_int
                })
            ...
            if result.get("success", False):
                ...
                total_balance = float(data.get("total_wallet_balance",
                ...
                available_balance = float(data.get("available_balance",
                                         ...
                                         data.get("futures_balance", 0))))

                total_unrealized_pnl = float(data.get("total_unrealized_pnl",
                                            data.get("totalUnrealizedPnl", 0)))
