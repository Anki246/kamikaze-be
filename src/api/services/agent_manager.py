...
class AgentManager:
    ...
    async def _get_user_credentials(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            mainnet_creds = await self.credentials_db.get_binance_credentials(user_id, is_mainnet=True)
            if mainnet_creds:
                credentials['binance_api_key'] = mainnet_creds['api_key']  # Already decrypted
                credentials['binance_secret_key'] = mainnet_creds['secret_key']  # Already decrypted
                credentials['is_mainnet'] = True
                self.logger.info(f"✅ Retrieved mainnet Binance credentials for user {user_id}")
                return credentials

            # Fallback to testnet credentials if mainnet not available
            testnet_creds = await self.credentials_db.get_testnet_credentials(user_id, "binance")
            if testnet_creds:
                credentials['binance_api_key'] = testnet_creds['api_key']  # Already decrypted
                credentials['binance_secret_key'] = testnet_creds['secret_key']  # Already decrypted
                credentials['is_mainnet'] = False
                self.logger.info(f"✅ Retrieved testnet Binance credentials for user {user_id}")
                return credentials
            ...
            self.logger.error(f"❌ Failed to retrieve credentials for user {user_id}: {e}")
            return {}
            ...
            if agent_id in self.agents:
                ...
                agents.append(AgentResponse(
                    ...
                    runtime=AgentRuntimeData(
                        uptime=agent.get_uptime() if hasattr(agent, 'get_uptime') else None,
        ...
        try:
            ...
            if user_id is None:
                try:
                    ...
                    if user_context:
                        ...
                        self.logger.info(f"🔧 Using user from context: {user_id} ({user_context.email})")
                    else:
                        self.logger.error("❌ No user_id provided and no user context available")
                        raise Exception("No user_id provided")
                except ImportError:
                    self.logger.error("❌ No user_id provided and user context not available")
                    raise Exception("No user_id provided")
            ...
            if user_id:
                ...
                if user_credentials:
                    ...
                    os.environ['BINANCE_SECRET_KEY'] = user_credentials['binance_secret_key']

                    config.update(user_credentials)
                    config['user_id'] = user_id
                    self.logger.info(f"✅ Added user {user_id} credentials to agent {agent_id} config and environment")
                    self.logger.info(f"🔍 Config now contains user_id: {config.get('user_id')}")
                else:
                    self.logger.error(f"❌ No credentials found for user {user_id}, agent cannot function")
    ...
    async def get_agent_status(self, agent_id: str) -> Optional[AgentStatusResponse]:
        ...
        if agent_id in self.agents:
            ...
            return AgentStatusResponse(
                ...
                uptime_seconds=agent.get_uptime() if hasattr(agent, 'get_uptime') else 0,
                current_cycle=getattr(agent, 'current_cycle', 0),
                max_cycles=getattr(agent, 'max_cycles', 100),
                last_activity=getattr(agent, 'last_activity', None) or datetime.utcnow(),
                mcp_connected=self._check_agent_mcp_connection(agent),
                binance_connected=self._check_agent_binance_connection(agent),
                groq_connected=self._check_agent_groq_connection(agent)
            )
    ...
    def _check_agent_mcp_connection(self, agent) -> bool:
        ...
        try:
            ...
            if hasattr(agent, 'binance_tools') and hasattr(agent.binance_tools, 'mcp_connected'):
                return agent.binance_tools.mcp_connected
            # Check if agent has MCP client and it's connected
            elif hasattr(agent, 'mcp_client') and hasattr(agent.mcp_client, 'connected'):
                return agent.mcp_client.connected
            elif hasattr(agent, 'binance_tools') and hasattr(agent.binance_tools, 'mcp_client') and hasattr(agent.binance_tools.mcp_client, 'connected'):
                return agent.binance_tools.mcp_client.connected
            elif hasattr(agent, 'trading_bot') and hasattr(agent.trading_bot, 'mcp_client') and hasattr(agent.trading_bot.mcp_client, 'connected'):
    ...
    def _check_agent_binance_connection(self, agent) -> bool:
        ...
        try:
            ...
            elif hasattr(agent, 'binance_tools') and hasattr(agent.binance_tools, 'connected'):
                return agent.binance_tools.connected
            elif hasattr(agent, 'trading_bot') and hasattr(agent.trading_bot, 'binance_tools') and hasattr(agent.trading_bot.binance_tools, 'connected'):
    ...
    def _check_agent_groq_connection(self, agent) -> bool:
        ...
        try:
            ...
            if hasattr(agent, 'groq_client'):
                return getattr(agent.groq_client, 'connected', True)  # Assume connected if client exists
            elif hasattr(agent, 'llm_client'):
                return getattr(agent.llm_client, 'connected', True)  # Assume connected if client exists
    ...
    async def get_agent_metrics(self, agent_id: str) -> Optional[TradingMetricsResponse]:
    ...
    async def get_agent_config(self, agent_id: str) -> Optional[AgentConfigResponse]:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Invalid config for agent {agent_id}, using defaults: {e}")
                configuration = AgentConfiguration()
