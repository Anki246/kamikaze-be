...
from agents.base_agent import BaseAgent, AgentMetadata, StrategyType, AgentStatus
from shared.constants import TRADING_PAIRS, API_ENDPOINTS, DEFAULT_CONFIG
from shared.utils import (
    generate_binance_signature, get_timestamp, format_currency,
    calculate_percentage_change, validate_trading_pair
)
...
class BinanceToolsInterface:
    ...
    def __init__(self, api_key: str = None, secret_key: str = None, user_id: int = None):
        ...
        try:
            ...
            async with self.__class__._connection_lock:
                ...
                try:
                    ...
                    if market_data_api and hasattr(market_data_api, 'mcp_client') and market_data_api.mcp_client:
                        self.logger.info("♻️  Reusing existing global MCP client from market_data_api")
                        ...
                        if not self.__class__._shared_mcp_client:
                            ...
                            self.__class__._shared_mcp_client = await create_binance_client()
                        else:
            ...
            if self.mcp_client:
                ...
                if self._user_id:
                    self.logger.info(f"🔑 Setting user credentials for user {self._user_id}...")
                    try:
                        creds_result = await self.call_tool("set_user_credentials_tool", {"user_id": self._user_id})
                        if creds_result.get("success"):
                            ...
                            self.logger.error(f"❌ Failed to set user credentials: {error_msg}")
                            self.mcp_connected = False
                            return
                ...
                if ping_result.get("status") == "healthy" or ping_result.get("result", {}).get("status") == "pong":
                    # Get detailed server status
                    try:
                        status_result = await self.mcp_client.call_tool("get_server_status")
                        if status_result.get("success"):
                            self.server_info = status_result

                            # Log server capabilities
                            self.logger.info("✅ Connected to Binance FastMCP Server")
                            self.logger.info(f"📡 Server: {self.server_info.get('server_name', 'Unknown')}")
                            self.logger.info(f"🔢 Version: {self.server_info.get('version', 'Unknown')}")

                            features = self.server_info.get("features", {})
                            self.logger.info(f"🔧 TA-Lib: {'✅' if features.get('talib_available') else '❌'}")
                            self.logger.info(f"🌐 WebSocket: {'✅' if features.get('websocket_available') else '❌'}")
                            self.logger.info(f"💱 Binance API: {'✅' if features.get('binance_configured') else '❌'}")

                            # Log available tools
                            tools = self.mcp_client.get_available_tools()
                            self.logger.info(f"🛠️  Available Tools: {len(tools)} tools ready")

                        else:
                            self.logger.warning("⚠️  Server status check failed, but connection established")
                    except Exception as status_e:
                        self.logger.warning(f"⚠️  Could not get server status: {status_e}")
    ...
    async def call_tool(self, tool_name: str, arguments: Dict = None) -> Dict:
        ...
        try:
            self.logger.debug(f"🔧 Calling FastMCP tool: {tool_name} with args: {arguments}")
            ...
            if result.get("success", True):  # Some tools don't return success flag
                ...
                self.logger.warning(f"⚠️  Tool '{tool_name}' returned error: {result.get('error', 'Unknown')}")
    ...
    async def get_24h_ticker(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result.get("success"):
                ...
                self.logger.info(f"✅ {symbol} Ticker: ${price} ({change}%) Vol: {volume}")
                return result
            else:
                error_msg = result.get('error', 'Unknown error')
                self.logger.warning(f"⚠️  FastMCP ticker call failed for {symbol}: {error_msg}")
                return None
        except Exception as e:
    ...
    async def place_futures_order(self, symbol: str, side: str, quantity: float, precision: int = 3, order_type: str = "MARKET", price: float = None) -> Optional[Dict]:
...
class FluxTraderAgent(BaseAgent):
    ...
    async def _broadcast_trading_event(self, event_data: Dict[str, Any]):
        ...
        await self._broadcast_event("trading_event", {
            ...
            try:
                await self.websocket_manager.broadcast_cycle_analysis(
                    self.agent_id,
                    {
                        ...
                        "performance_metrics": cycle_data.get("performance_metrics", {}),
            ...
            try:
                await self.websocket_manager.broadcast_trade_execution(
                    self.agent_id,
                    {
                        ...
                        "status": trade_data.get("status"),  # PENDING/FILLED/CANCELLED/FAILED
    ...
    def __init__(self, agent_id: str, config_dict: Dict[str, Any]):
        ...
        self.logger.info(f"🔍 Agent config - user_id: {user_id}, api_key: {'***' if api_key else None}, secret_key: {'***' if secret_key else None}")

        self.binance_tools = BinanceToolsInterface(api_key=api_key, secret_key=secret_key, user_id=user_id)
        ...
        self.trade_amount_usdt = float(os.getenv("TRADE_AMOUNT_USDT", str(trading_params["trade_amount"])))
        self.max_position_size_pct = float(os.getenv("MAX_POSITION_SIZE_PCT", "2.0"))
        self.pump_threshold = float(os.getenv("PUMP_THRESHOLD", str(trading_params["signal_threshold"])))
        self.dump_threshold = float(os.getenv("DUMP_THRESHOLD", str(-trading_params["signal_threshold"])))
        self.min_confidence = int(os.getenv("MIN_CONFIDENCE", "35"))
        self.signal_strength_threshold = float(os.getenv("SIGNAL_STRENGTH_THRESHOLD", "0.4"))
        ...
        self.trailing_stop_loss_1 = float(os.getenv("TRAILING_STOP_LOSS_1", str(trading_params["trailing_stop_loss_1"])))
        self.trailing_stop_loss_2 = float(os.getenv("TRAILING_STOP_LOSS_2", str(trading_params["trailing_stop_loss_2"])))
        self.trailing_stop_loss_3 = float(os.getenv("TRAILING_STOP_LOSS_3", str(trading_params["trailing_stop_loss_3"])))

        self.trailing_take_profit_1 = float(os.getenv("TRAILING_TAKE_PROFIT_1", str(trading_params.get("trailing_take_profit_1", 2.0))))
        self.trailing_take_profit_2 = float(os.getenv("TRAILING_TAKE_PROFIT_2", str(trading_params.get("trailing_take_profit_2", 3.5))))
        self.trailing_take_profit_3 = float(os.getenv("TRAILING_TAKE_PROFIT_3", str(trading_params.get("trailing_take_profit_3", 6.0))))
        ...
        self.trading_mode = os.getenv("TRADING_MODE", "REAL" if config.app.real_trading else "SIMULATION")
    ...
    async def get_multi_timeframe_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            correlation_strength = "HIGH" if abs(btc_change - symbol_change) < 0.5 else "LOW"
    ...
    async def ai_analyze_signal_holistic(self, symbol: str, signal_data: Dict, multi_tf_data: Dict) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            print(f"🔬 Fetching professional technical analysis for {symbol} via TRUE MCP...")
            ...
            sr_levels = await self.binance_tools.call_tool("calculate_support_resistance_levels", {
                "symbol": symbol,
                "timeframe": "1h",
                "periods": 20
            })
            ...
            tech_indicators = await self.binance_tools.call_tool("calculate_technical_indicators", {
                "symbol": symbol,
                "timeframe": "1h",
            ...
            correlation_data = await self.binance_tools.call_tool("analyze_market_correlation", {
                "symbol": symbol,
                "reference_symbols": ["BTCUSDT", "ETHUSDT"]
            })

            # Get Market Sentiment via MCP
            sentiment_data = await self.binance_tools.call_tool("assess_market_sentiment", {
                "symbol": symbol
            })

            # Get Multi-timeframe Data via MCP
            mtf_data = await self.binance_tools.call_tool("get_multi_timeframe_data", {
                "symbol": symbol,
                "timeframes": ["1m", "5m", "15m", "1h", "4h", "1d"]
            })

            if sr_levels.get("error") or tech_indicators.get("error"):
                print(f"⚠️  MCP Technical Analysis partially failed - using fallback data")
            else:
                print(f"✅ Professional technical analysis data collected via TRUE MCP for {symbol}")

            # Calculate market sentiment indicators
            volume_strength = "HIGH" if volume > 500000 else "MEDIUM" if volume > 100000 else "LOW"
            volatility_level = "HIGH" if abs(price_change_pct) > 2.0 else "MEDIUM" if abs(price_change_pct) > 0.5 else "LOW"
            ...
            mcp_support_levels = sr_levels.get("support_levels", []) if sr_levels.get("success") else []
            mcp_resistance_levels = sr_levels.get("resistance_levels", []) if sr_levels.get("success") else []
            mcp_support_level = mcp_support_levels[0] if mcp_support_levels else support_level
            mcp_resistance_level = mcp_resistance_levels[0] if mcp_resistance_levels else resistance_level
            ...
            mcp_risk_reward_ratio = mcp_reward_distance / mcp_risk_distance if mcp_risk_distance > 0 else risk_reward_ratio

            # Extract BTC correlation from MCP data
            mcp_btc_correlation = correlation_data.get("btc_correlation", "UNKNOWN") if correlation_data.get("success") else correlation_strength

            # Extract market sentiment level
            mcp_sentiment_level = sentiment_data.get("overall_sentiment", "UNKNOWN") if sentiment_data.get("success") else "UNKNOWN"

            # Extract market regime
            mcp_market_regime = correlation_data.get("market_regime", "UNKNOWN") if correlation_data.get("success") else "UNKNOWN"
            ...
            **Detailed Reasoning:**
            ...
            print(f"   🎯 Support/Resistance Levels: {len(sr_levels.get('consolidated_levels', []) if sr_levels.get('success') else [])} levels identified")
            print(f"   📈 Technical Indicators: {len(tech_indicators.get('indicators', {}) if tech_indicators.get('success') else {})} calculated")
                    ...
                    for match in matches:
                        try:
                            ...
                            if 0.0 <= confidence_value <= 1.0:
                                ...
                                print(f"   🔍 EXTRACTED CONFIDENCE: {confidence_value * 100:.1f}%")
                                break
                        except ValueError:
                            continue
                if confidence_found:
            ...
            print(f"   🔬 Technical Factors: Support/Resistance, Indicators, Correlation")
    ...
    def _extract_indicators_summary(self, indicators: Dict) -> str:
        ...
        try:
            ...
            if "RSI" in indicator_data:
                ...
                status = "OVERBOUGHT" if overbought else "OVERSOLD" if oversold else "NEUTRAL"
            ...
            if "MACD" in indicator_data:
                ...
                crossover = "BULLISH CROSSOVER" if bullish else "BEARISH CROSSOVER" if bearish else "NO CROSSOVER"
            ...
            if "BOLLINGER_BANDS" in indicator_data:
                ...
                position_desc = "UPPER" if bb_position > 0.8 else "LOWER" if bb_position < 0.2 else "MIDDLE"
            ...
            if overall:
                ...
                summary += f"  Overall Signal: {signal} (Confidence: {confidence:.1f}%)\n"
    ...
    async def calculate_trade_size(self, symbol: str, current_price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if symbol_info:
                ...
                for filter_info in symbol_info.get('filters', []):
                    if filter_info['filterType'] == 'LOT_SIZE':
                        ...
                        min_notional = float(filter_info.get('minNotional', min_notional))
            ...
            if final_notional < min_notional:
                print(f"   ❌ VALIDATION FAILED: Notional ${final_notional:.2f} < Min ${min_notional:.2f}")
                return None
            ...
            print(f"      💵 Final Notional: ${final_notional:.2f} (>= ${min_notional:.2f})")
            print(f"      📦 Final Quantity: {quantity:.{quantity_precision}f} (>= {min_qty:.8f})")
    ...
    async def execute_real_trade_mcp(self, symbol: str, action: str, current_price: float, ai_confidence: float = 0.75) -> bool:
        ...
        try:
            ...
            if fresh_balance.get("success"):
                ...
                self.logger.info(f"✅ Fresh Balance - Available: ${self.available_balance:.8f}, Total: ${self.account_balance:.8f}, PnL: ${unrealized_pnl:+.8f}")
            else:
                self.logger.warning(f"⚠️  Could not refresh balance, using cached: ${self.available_balance:.2f}")
            ...
            if self.available_balance < required_margin:
                ...
                self.logger.error("🚨 CRITICAL ERROR: INSUFFICIENT BALANCE - TRADE REJECTED!")
                self.logger.error("🚨" * 60)
                self.logger.error(f"   💰 Available Balance: ${self.available_balance:.2f} USDT")
                self.logger.error(f"   🛡️  Required Margin: ${required_margin:.2f} USDT")
                self.logger.error(f"   💸 Shortfall: ${required_margin - self.available_balance:.2f} USDT")
                self.logger.error(f"   📊 Trade Value: ${trade_data['trade_value_usdt']:.2f} USDT")
                self.logger.error(f"   ⚡ Leverage: {self.leverage}x")
                self.logger.error("")
                self.logger.error("🔧 SOLUTION: Add more USDT to your Binance Futures account")
                self.logger.error(f"   💵 Minimum needed: ${required_margin:.2f} USDT")
                self.logger.error(f"   💰 Recommended: ${required_margin * 2:.2f} USDT (for multiple trades)")
                self.logger.error("🚨" * 60)
                return False
            ...
            self.logger.info(f"   💵 Remaining: ${self.available_balance - required_margin:.2f}")

            quantity = trade_data["quantity"]
            precision = trade_data["quantity_precision"]

            self.logger.warning("💼 EXECUTING REAL BINANCE FUTURES TRADE VIA ENHANCED FASTMCP")
            ...
            self.logger.warning(f"💵 Trade Value: ${trade_data['trade_value_usdt']:.2f} USDT")
            self.logger.warning(f"🛡️  Margin Required: ${trade_data['margin_required']:.2f} USDT")
            ...
            # Set leverage first
            leverage_success = await self.binance_tools.set_leverage(symbol, self.leverage)
            if leverage_success:
            ...
            if action == "BUY":
                ...
                take_profit_price = current_price * (1 + self.trailing_take_profit_1 / 100)
            ...
            print(f"\n🎯 CALCULATED LEVELS:")
            print(f"   🛑 Stop Loss: ${stop_loss_price:.2f} ({self.trailing_stop_loss_1}%)")
            print(f"   🎯 Take Profit: ${take_profit_price:.2f} ({self.trailing_take_profit_1}%)")
            ...
            order_result = await self.binance_tools.place_futures_order(symbol, side, quantity, precision, "MARKET")
            ...
            print(f"   💵 Avg Price: ${float(order_result.get('avgPrice', current_price)):.2f}")
            ...
            stop_order = await self.binance_tools.place_stop_loss_order(symbol, side, quantity, stop_loss_price)
            ...
            print(f"   🆔 Order ID: {order_result.get('orderId')}")
            print(f"   💰 Actual Fill Price: ${float(order_result.get('avgPrice', current_price)):.2f}")
            print(f"   📦 Actual Quantity: {float(order_result.get('executedQty', quantity)):.{precision}f}")
    ...
    async def run_simplified_mcp_trading(self, safety_confirmation=None):
        ...
        print(f"   - Signal Strength: >{self.signal_strength_threshold} (ULTRA AGGRESSIVE)")
        ...
        print(f"💰 Trading Mode: {'REAL MONEY' if self.enable_real_trades else 'SIMULATION'}")
        print(f"⚡ Leverage: {self.leverage}x")
        print(f"🔬 ULTRA AGGRESSIVE + AI + MCP - Detects micro-movements with AI validation")
        print("=" * 80)
        ...
        print(f"⚡ Leverage: {self.leverage}x")
        print(f"🛡️  Margin per Trade: ${self.trade_amount_usdt / self.leverage:.2f} USDT")
        ...
        if self.enable_real_trades:
            if safety_confirmation:
                ...
                confirmation = input("🚨 Type 'EXECUTE-REAL-MCP-TRADES' to start LIVE trading with REAL MONEY: ")
            ...
            if safety_confirmation:
                ...
                # Fallback to terminal input (for direct bot execution)
                confirmation = input("🚨 Type 'EXECUTE-SIMULATION-MCP' to start simulation trading: ")
        ...
        print(f"⚡ Leverage: {self.leverage}x")
        print(f"🛡️  Margin per Trade: ${self.trade_amount_usdt / self.leverage:.2f} USDT")
        print(f"📊 Trading Pairs: {len(self.trading_pairs)} pairs ({', '.join(self.trading_pairs[:4])}...)")
        ...
        try:
            for cycle in range(self.max_cycles):  # Full cycles for real trading
                ...
                cycle_msg = f"🔄 FLUXTRADER MCP TRADING CYCLE {cycle + 1}/{self.max_cycles}"
                ...
                for symbol in self.trading_pairs[:3]:  # Limited pairs for demo
                    try:
                        ...
                        try:
                            ...
                            current_price = safe_float(ticker_data.get("price", ticker_data.get("lastPrice", 0)))
                            price_change_pct = safe_float(ticker_data.get("change_percent_24h", ticker_data.get("priceChangePercent", 0)))
                            volume = safe_float(ticker_data.get("volume_24h", ticker_data.get("volume", 0)))
                            high_24h = safe_float(ticker_data.get("high_24h", ticker_data.get("highPrice", current_price if current_price > 0 else 1)))
                            low_24h = safe_float(ticker_data.get("low_24h", ticker_data.get("lowPrice", current_price if current_price > 0 else 1)))
                        ...
                        if len(self.price_history[symbol]) >= 3:
                            recent_prices = [p["price"] for p in self.price_history[symbol]]
                        ...
                        if high_24h != low_24h and high_24h > 0 and low_24h > 0:
                            ...
                            price_position = 50  # Default to middle position if no range

                        volume_factor = min(4.0, volume / 200000) if volume > 0 else 0  # Ultra aggressive volume sensitivity

                        # Calculate price volatility for micro-movement detection with error handling
                        if current_price > 0 and high_24h > low_24h:
                            price_range_pct = ((high_24h - low_24h) / current_price) * 100
                            volatility_boost = min(0.5, price_range_pct / 8)  # Increased boost for any volatility
                        else:
                            price_range_pct = 0
                            volatility_boost = 0
                        ...
                        if len(self.price_history[symbol]) >= 3:
                            recent_prices = [p["price"] for p in self.price_history[symbol][-3:]]
                        ...
                        position_boost = 0.3 if (price_position > 70 and momentum > 0) or (price_position < 30 and momentum < 0) else 0.1
                        micro_movement_boost = 0.2  # Base boost for any detected movement

                        signal_strength = base_strength + volume_boost + position_boost + volatility_boost + micro_trend_boost + micro_movement_boost

                        # DETAILED CONSOLE OUTPUT - Technical Analysis Breakdown - PRESERVED
                        print(f"\n🔬 ULTRA-AGGRESSIVE TECHNICAL ANALYSIS BREAKDOWN:")
                        print(f"   📍 Price Position in 24h Range: {price_position:.1f}%")
                        print(f"   📊 Volume Factor: {volume_factor:.2f} (threshold: 200k)")
                        print(f"   📈 Price Range (24h): {price_range_pct:.2f}%")
                        print(f"   🌊 Volatility Boost: {volatility_boost:.3f}")
                        print(f"   📊 Micro-Trend: {trend_direction} (boost: {micro_trend_boost:.2f})")

                        print(f"\n💪 SIGNAL STRENGTH CALCULATION:")
                        print(f"   🔢 Base Strength: {base_strength:.3f} = |{momentum:.3f}| * 2 + |{price_change_pct:.3f}| * 0.8")
                        print(f"   📊 Volume Boost: {volume_boost:.3f} = {volume_factor:.2f} * 0.6")
                        ...
                        print(f"   💪 Signal Strength Threshold: {self.signal_strength_threshold}")
                        ...
                        print(f"   ✅ Momentum ≥ {self.pump_threshold:.3f}%: {momentum:.4f}% {'✅ PASS' if pump_conditions[0] else '❌ FAIL'}")
                        print(f"   ✅ 24h Change > {self.min_24h_change:.3f}%: {abs(price_change_pct):.4f}% {'✅ PASS' if pump_conditions[1] else '❌ FAIL'}")
                        print(f"   ✅ Signal Strength > {self.signal_strength_threshold}: {signal_strength:.4f} {'✅ PASS' if pump_conditions[2] else '❌ FAIL'}")

                        print(f"\n📉 DUMP SIGNAL DETECTION:")
                        print(f"   ✅ Momentum ≤ {self.dump_threshold:.3f}%: {momentum:.4f}% {'✅ PASS' if dump_conditions[0] else '❌ FAIL'}")
                        print(f"   ✅ 24h Change > {self.min_24h_change:.3f}%: {abs(price_change_pct):.4f}% {'✅ PASS' if dump_conditions[1] else '❌ FAIL'}")
                        print(f"   ✅ Signal Strength > {self.signal_strength_threshold}: {signal_strength:.4f} {'✅ PASS' if dump_conditions[2] else '❌ FAIL'}")
                        ...
                        if all(pump_conditions):
                            ...
                            ai_analysis = await self.ai_analyze_signal_holistic(symbol, signal_data, multi_tf_data)

                            # Check AI confirmation with holistic analysis
                            if ai_analysis["ai_decision"] in ["BUY", "NEUTRAL"] and ai_analysis["ai_confidence"] > 0.3:
                                print(f"\n🤖 ═══════════════════════════════════════════════════════════════")
                                print(f"🤖 FLUXTRADER AI AGENT - COMPREHENSIVE DECISION ANALYSIS")
                                print(f"🤖 ═══════════════════════════════════════════════════════════════")
                                print(f"")
                                print(f"🧠 AI AGENT PROFILE:")
                                print(f"   🎯 Agent Type: FluxTrader AI Trading Agent")
                                print(f"   🔬 Analysis Model: Groq LLM (llama3-8b-8192)")
                                print(f"   📊 Decision Framework: Multi-Factor Holistic Analysis")
                                print(f"   🎛️  Temperature: 0.1 (Conservative, Precise)")
                                print(f"   📝 Max Tokens: 400 (Detailed Analysis)")
                                print(f"   🔄 Processing Mode: Real-time Market Analysis")
                                print(f"")
                                print(f"🔍 AI DECISION-MAKING PARAMETERS:")
                                print(f"   📈 Signal Type: PUMP (BUY Signal)")
                                print(f"   ⚡ Momentum Threshold: ≥+0.030% (ULTRA AGGRESSIVE)")
                                ...
                                print(f"   🔬 Technical Analysis: Multi-timeframe (1m-1d)")
                                print(f"   🌐 Market Correlation: BTC, ETH, Traditional Assets")
                                print(f"   😱 Sentiment Analysis: Fear/Greed, Funding, Social")
                                print(f"")
                                print(f"🎯 CURRENT SIGNAL ANALYSIS:")
                                print(f"   ✅ Momentum: {momentum:+.4f}% (Threshold: +0.030%)")
                                print(f"   ✅ Signal Strength: {signal_strength:.3f} (Threshold: 0.4)")
                                print(f"   ✅ 24h Change: {price_change_pct:+.3f}% (Threshold: ±0.010%)")
                                print(f"   ✅ Volume: {volume:,.0f} (Factor: {signal_data.get('volume_factor', 1.0):.2f})")
                                print(f"   ✅ Micro-Trend: {trend_direction}")
                                print(f"")
                                print(f"🤖 AI DECISION OUTPUT:")
                                print(f"   🎯 AI Decision: {ai_analysis['ai_decision']}")
                                print(f"   🔥 AI Confidence: {ai_analysis['ai_confidence'] * 100:.1f}%")
                                print(f"   ⚠️  Risk Assessment: {ai_analysis['risk_level']}")
                                print(f"   📊 Support Level: ${ai_analysis.get('support_level', 0):.2f}")
                                print(f"   📊 Resistance Level: ${ai_analysis.get('resistance_level', 0):.2f}")
                                print(f"   ⚖️  Risk/Reward Ratio: {ai_analysis.get('risk_reward_ratio', 0):.2f}")
                                print(f"")
                                print(f"🧠 AI LOGICAL REASONING:")
                                # Display FULL reasoning for trade execution
                                reasoning = ai_analysis.get('ai_reasoning', 'No reasoning provided')
                                if reasoning:
                                    ...
                                    print(f"   💭 No reasoning provided")
                                print(f"")
                                print(f"✅ AI CONFIRMS TRADE EXECUTION (HOLISTIC ANALYSIS COMPLETE)")
                                print(f"🤖 ═══════════════════════════════════════════════════════════════")
                                ...
                                if self.enable_real_trades:
                                    trade_success = await self.execute_real_trade_mcp(symbol, action, current_price, ai_confidence)
                                    if trade_success:
                                    ...
                                    print(f"   💵 Trade Amount: ${self.trade_amount_usdt:.2f}")
                                ...
                                print(f"💡 Reason: Low confidence or unfavorable conditions")
                            ...
                            # Check AI confirmation with holistic analysis
                            if ai_analysis["ai_decision"] in ["SELL", "NEUTRAL"] and ai_analysis["ai_confidence"] > 0.3:
                                print(f"\n🤖 ═══════════════════════════════════════════════════════════════")
                                print(f"🤖 FLUXTRADER AI AGENT - COMPREHENSIVE DECISION ANALYSIS")
                                print(f"🤖 ═══════════════════════════════════════════════════════════════")
                                print(f"")
                            ...
                            print(f"\n⚪ NO SIGNAL DETECTED")
                            print(f"   📊 Analysis Complete - No trading opportunity found")
                ...
                pairs_summary = ", ".join([f"{pair['symbol']}: ${pair['price']:,.2f} ({pair['change_pct']:+.2f}%)" for pair in cycle_pairs_data])
                ...
                print(f"   🤖 AI Analysis: {'ACTIVE' if self.groq_client else 'INACTIVE'}")
                ...
                if cycle_pairs_data:
                    ...
                    for pair in cycle_pairs_data:
                        print(f"      {pair['symbol']}: ${pair['price']:,.2f} ({pair['change_pct']:+.2f}%) | Vol: {pair['volume']:,.0f}")
                ...
                await self._broadcast_cycle_analysis({
                    ...
                    "status": "completed",
                    "pairs_analyzed": cycle_pairs_data,
                    "signals_detected": [pair for pair in cycle_pairs_data if signals_found > 0],
                    "market_conditions": {
                        "total_pairs_analyzed": len(cycle_pairs_data),
                        "signals_found": signals_found,
                        "cycle_duration": (cycle_end_time - datetime.now()).total_seconds() if hasattr(self, 'cycle_start_time') else 0,
                        "average_price_change": sum([pair.get('change_pct', 0) for pair in cycle_pairs_data]) / len(cycle_pairs_data) if cycle_pairs_data else 0,
                        "highest_volume_pair": max(cycle_pairs_data, key=lambda x: x.get('volume', 0)) if cycle_pairs_data else None,
                        "most_volatile_pair": max(cycle_pairs_data, key=lambda x: abs(x.get('change_pct', 0))) if cycle_pairs_data else None
                    },
                    "performance_metrics": {
                        "current_balance": self.available_balance,
                        "signals_detection_rate": (signals_found / len(cycle_pairs_data)) * 100 if cycle_pairs_data else 0,
                        "cycle_efficiency": signals_found / max(1, len(self.trading_pairs)) * 100
                    },
                    ...
                    "next_cycle_eta": (datetime.now() + timedelta(seconds=30)).isoformat() if cycle + 1 < self.max_cycles else None
                })
    ...
    def get_metadata(self) -> AgentMetadata:
        ...
        return AgentMetadata(
            ...
            features=["ai_analysis", "multi_level_stops", "real_time_trading", "mcp_integration"]
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if not user_id:
                    self.logger.error("❌ No user_id in agent config - cannot test API connection")
                    self._set_status(AgentStatus.ERROR)
                    return False

                balance_result = await market_data_api.get_account_balance(user_id=user_id)
    ...
    async def get_balance(self) -> Dict[str, float]:
        ...
        try:
            ...
            try:
                ...
                user_id = self.config.get('user_id')
                if not user_id:
                    self.logger.warning("⚠️ No user_id in agent config - using mock balance data")
                    return {
                        "total_balance": 0.0,
                        "available_balance": 0.0,
                        "unrealized_pnl": 0.0,
                ...
                if balance_result.get("success", False):
                    return {
                        ...
                        "available_balance": balance_result.get("available_balance", 0.0),
                        "unrealized_pnl": balance_result.get("total_unrealized_pnl", 0.0),
    ...
    async def _trading_loop(self):
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if self.binance_tools.mcp_connected:
                        self.logger.info(f"✅ MCP connection established on attempt {attempt + 1}")
                        break
                    else:
                        self.logger.warning(f"⚠️ MCP connection failed on attempt {attempt + 1}")
                except Exception as e:
                    self.logger.warning(f"⚠️ MCP connection error on attempt {attempt + 1}: {e}")

                if attempt < max_retries - 1:
                    await asyncio.sleep(2 ** attempt)  # Exponential backoff

            if not self.binance_tools.mcp_connected:
                error_msg = f"Failed to connect to MCP server after {max_retries} attempts"
                self.logger.error(f"❌ {error_msg}")
                await self._broadcast_trading_event({
                    "type": "error",
                    "message": error_msg,
            ...
            self.logger.info(f"✅ Credentials verified - Account balance: ${balance_test.get('total_wallet_balance', 0):.8f}")

            # Run the existing FluxTrader trading logic with automatic confirmation
            await self.run_simplified_mcp_trading(safety_confirmation="EXECUTE-REAL-MCP-TRADES")
        except asyncio.CancelledError:
...
async def main():
    ...
    try:
        ...
        try:
            cycle_count = 0
            while agent._running and hasattr(agent, '_task') and agent._task and not agent._task.done():
                ...
                if cycle_count % 12 == 0:
                    ...
                    print(f"   Available Balance: ${agent.available_balance:.8f}")
                    print(f"   Task Done: {agent._task.done() if agent._task else 'No Task'}")
