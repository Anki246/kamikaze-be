...
async def load_configuration(force_reload: bool = False) -> Dict[str, Any]:
    ...
    if use_aws_secrets and AWS_SECRETS_AVAILABLE:
        ...
        try:
            ...
            try:
                ...
                logger.debug("✅ Database configuration loaded from AWS Secrets Manager")
            except Exception as e:
                aws_errors.append(f"Database config: {e}")
                logger.warning(f"⚠️ Failed to load database config from AWS Secrets Manager: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Failed to load API keys from AWS Secrets Manager: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Failed to load application secrets from AWS Secrets Manager: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Failed to load AWS credentials from AWS Secrets Manager: {e}")

            if config_data:
                logger.info(f"✅ Partial configuration loaded from AWS Secrets Manager ({len(config_data)} settings)")
                if aws_errors:
                    logger.warning(f"⚠️ Some secrets failed to load: {len(aws_errors)} errors")
    ...
    if not use_aws_secrets:
        ...
        env_vars = [
            ...
            "DB_MIN_SIZE", "DB_MAX_SIZE", "DB_TIMEOUT",
            "POSTGRES_HOST", "POSTGRES_PORT", "POSTGRES_DATABASE", "POSTGRES_USER", "POSTGRES_PASSWORD",
            
            # API Keys
            "BINANCE_API_KEY", "BINANCE_SECRET_KEY", "GROQ_API_KEY", "OPENAI_API_KEY",
            
            # Application secrets
            "JWT_SECRET", "ENCRYPTION_KEY", "CREDENTIALS_ENCRYPTION_KEY", "WEBHOOK_SECRET",
            
            # AWS
            "AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_REGION", "AWS_DEFAULT_REGION", "AWS_SESSION_TOKEN",
            
            # Environment
            "ENVIRONMENT", "USE_AWS_SECRETS",
            
            # Trading configuration
            "LEVERAGE", "TRADE_AMOUNT_USDT", "MAX_POSITION_SIZE_PCT", "PUMP_THRESHOLD",
            "DUMP_THRESHOLD", "MIN_CONFIDENCE", "SIGNAL_STRENGTH_THRESHOLD", "MIN_24H_CHANGE",
            "MAX_CYCLES", "ALLOCATION_STRATEGY", "MIN_TRADE_AMOUNT",
        ]
...
def should_use_aws_secrets() -> bool:
    ...
    return (
        is_production() or 
        get_config_value("USE_AWS_SECRETS", "false", bool)
    )
