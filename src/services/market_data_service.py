...
from infrastructure.event_bus import event_bus, publish_market_data, EventType, BaseEvent
...
class MarketDataService:
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Market Data Service started for {len(self.symbols)} symbols")
    ...
    async def _start_symbol_stream(self, symbol: str):
        ...
        while self.running:
            try:
                ...
                if self.running:
                    self.logger.info(f"🔄 Reconnecting to {symbol} in {reconnect_delay}s...")
                        ...
                        if len(recent_prices) >= 2:
                            ...
                            if old_price > 0:
                                ...
                                if abs(short_change) > 0.01:
                                    ...
                                    await event_bus.publish(f"trading:market_data:{symbol}", change_event)
