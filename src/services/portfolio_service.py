...
class PortfolioService:
    ...
    async def get_portfolio_metrics(self, user_id: int, realtime: bool = False, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._can_make_request(user_id):
                logger.warning(f"Request rate limit reached for user {user_id}, using cached data")
                if cache_key in self.cache:
                    return self.cache[cache_key]
                else:
                    return self._get_mock_portfolio_data()
            ...
            async with self.user_locks[user_id]:
                ...
                if self._is_cache_valid(cache_key, realtime):
                    logger.debug(f"Using cached portfolio data for user {user_id} (after lock)")
                    return self.cache[cache_key]
                ...
                portfolio_data = await self._calculate_portfolio_metrics(balances, prices)

            logger.debug(f"Portfolio data calculated for user {user_id}: {portfolio_data.get('total_value_usd', 0):.2f} USD")

            # Cache the result
            self.cache[cache_key] = portfolio_data
            self.last_update[cache_key] = datetime.now(timezone.utc)

            logger.info(f"Portfolio metrics calculated for user {user_id}: ${portfolio_data['total_value_usd']:.2f}")
            return portfolio_data
            
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Failed to get portfolio metrics for user {user_id}: {error_msg}")
            ...
            if "banned until" in error_msg or "request weight" in error_msg.lower():
                ...
                cache_key = f"portfolio_{user_id}"
                if cache_key in self.cache:
                    logger.info(f"Returning cached data for rate-limited user {user_id}")
                    return self.cache[cache_key]
                else:
    ...
    async def _get_user_credentials(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            credentials = await credentials_db.get_binance_credentials(user_id, is_mainnet=True)
            if not credentials:
                credentials = await credentials_db.get_binance_credentials(user_id, is_mainnet=False)
    ...
    async def _get_account_balances(self, credentials: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with service:
                ...
                if not spot_result['success']:
                    raise Exception(f"Failed to get spot account data: {spot_result['error']}")
                ...
                try:
                    ...
                    if futures_result['success']:
                        ...
                        for asset_data in futures_result['data']['assets']:
                            ...
                            if wallet_balance > 0:
                                ...
                                for existing in balances:
                                    if existing['asset'] == asset_data['asset'] and existing['account_type'] == 'spot':
                        ...
                        logger.debug(f"Retrieved {futures_balances_count} non-zero futures balances")
                    else:
                        logger.warning(f"Failed to get futures account data: {futures_result.get('error', 'Unknown error')}")

                except Exception as e:
                    logger.warning(f"Futures account not accessible or error occurred: {e}")
                    # Continue without futures data - not all accounts have futures enabled

                total_balances = len(balances)
                logger.info(f"Retrieved {total_balances} total non-zero balances (spot + futures)")
                return balances
    ...
    async def _get_current_prices(self, balances: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    async def _calculate_portfolio_metrics(self, balances: List[Dict[str, Any]], prices: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Asset price changes retrieved: {len(asset_price_changes)} assets")
            ...
            for balance in balances:
                ...
                btc_value = float(Decimal(str(usd_value)) / Decimal(str(btc_price_usd))) if btc_price_usd > 0 else 0.0
                ...
                if usd_value > 0:  # Only include assets with value
                    ...
                    logger.info(f"Asset {asset}: price_change_24h = {price_change_24h}% (from asset_price_changes)")
            ...
            for allocation in asset_allocation:
                allocation['percentage'] = (allocation['usd_value'] / total_value_usd * 100) if total_value_usd > 0 else 0.0

            # Sort by USD value (descending)
            asset_allocation.sort(key=lambda x: x['usd_value'], reverse=True)

            # Calculate daily P&L using 24hr price changes
            daily_pnl, daily_pnl_percent = await self._calculate_daily_pnl(balances, prices, total_value_usd)
    ...
    async def _get_asset_price_changes(self) -> Dict[str, float]:
        ...
        try:
            ...
            if (self.price_changes_last_update and
                self.price_changes_cache and
                (now - self.price_changes_last_update).total_seconds() < 300):  # 5 minutes cache
                logger.debug(f"Using cached price changes for {len(self.price_changes_cache)} assets")
            ...
            async with service:
                ...
                if not ticker_result['success']:
                    logger.warning(f"Failed to get 24hr ticker data for asset changes: {ticker_result['error']}")
                    # Return cached data if available, even if stale
                    if self.price_changes_cache:
                        logger.info(f"Using stale cached price changes due to API failure")
                        return self.price_changes_cache
                    return {}
                ...
                logger.debug(f"Retrieved and cached price changes for {len(price_changes)} assets")
    ...
    async def _calculate_daily_pnl(self, balances: List[Dict[str, Any]], current_prices: Dict[str, float], current_total_usd: float) -> Tuple[float, float]:
        ...
        try:
            ...
            async with service:
                ...
                if not ticker_result['success']:
                    logger.warning(f"Failed to get 24hr ticker data for P&L calculation: {ticker_result['error']}")
                    return 0.0, 0.0
                ...
                for ticker in ticker_result['data']:
                    if ticker['symbol'].endswith('USDT'):
                        ...
                        try:
                            price_change_percent = float(ticker['priceChangePercent'])
                            price_changes[asset] = price_change_percent / 100.0  # Convert to decimal
                        except (ValueError, KeyError):
                            continue
                ...
                daily_pnl_percent = (total_pnl_usd / yesterday_total_usd * 100) if yesterday_total_usd > 0 else 0.0

                logger.debug(f"Daily P&L calculated: ${total_pnl_usd:.2f} ({daily_pnl_percent:.2f}%)")
    ...
    def _handle_rate_limit_error(self, user_id: int, error_msg: str):
        ...
        try:
            ...
            if match:
                ...
                ban_until = (datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp() * 1000
                self.rate_limit_until[user_id] = ban_until
    ...
    async def get_recent_trades(self, user_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with service:
                ...
                if not account_result['success']:
                    logger.warning(f"Failed to get account data for trades: {account_result['error']}")
                    return []
    ...
    async def get_top_performers(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with service:
                ...
                if not ticker_result['success']:
                    logger.warning(f"Failed to get 24hr ticker data: {ticker_result['error']}")
                    return []
                        ...
                        try:
                            ...
                            if volume > 1000 and price > 0.001:
                                usdt_pairs.append({
                                    ...
                                    'name': ticker['symbol'].replace('USDT', ''),  # Could be enhanced with coin names
                                    'price': price,
