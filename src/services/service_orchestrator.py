...
from infrastructure.event_bus import event_bus, EventType, BaseEvent
    ...
    async def _handle_health_event(self, event: BaseEvent):
        ...
        try:
            if event.event_type == EventType.HEALTH_CHECK:
                ...
                self.logger.debug(f"📊 Health update from {service_name}: {event.data.get('status')}")
            ...
            try:
                ...
                for service_name, health_data in self.service_health.items():
                    ...
                    if last_update:
                        ...
                        if time_diff > self.alert_thresholds["max_downtime"]:
                            unhealthy_services.append(f"{service_name} (no update for {time_diff:.0f}s)")
                ...
                system_health = BaseEvent(
                    ...
                    data={
                        ...
                        "healthy_services": len(self.service_health) - len(unhealthy_services)
                    }
                )
...
class ServiceOrchestrator:
    ...
    async def start(self):
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(self._run_service("market_data", self.market_data_service)),
                asyncio.create_task(self._run_service("strategy_engine", self.strategy_engine)),
                asyncio.create_task(self._run_service("order_manager", self.order_manager)),
                asyncio.create_task(self._run_service("health_monitor", self.health_monitor))
            ]
        ...
        while self.running and restart_count < max_restarts:
            try:
                ...
                if restart_count < max_restarts and self.running:
                    wait_time = min(restart_count * 10, 60)  # Exponential backoff, max 60s
                    self.logger.info(f"🔄 Restarting {name} in {wait_time}s (attempt {restart_count}/{max_restarts})")
    ...
    async def _start_mcp_server(self):
        ...
        try:
            mcp_server_path = os.path.join(
                ...
                "mcp_servers", "binance_fastmcp_server.py"
            )
