...
current_user_credentials = {"api_key": None, "secret_key": None, "user_id": None}
...
class TechnicalAnalysisInput(BaseModel):
    ...
    indicators: Optional[List[str]] = Field(
        default=[], description="List of technical indicators to calculate"
    )
...
class SupportResistanceInput(BaseModel):
    ...
    lookback_periods: int = Field(
        default=100, description="Number of periods to look back"
    )
...
def set_cached_data(cache_key: str, data: Dict[str, Any]) -> None:
    ...
    request_cache[cache_key] = {"data": data, "timestamp": time.time()}
...
async def get_24h_ticker(input: TickerInput) -> Dict[str, Any]:
    ...
    try:
        result = await make_binance_request(
            "/fapi/v1/ticker/24hr", params={"symbol": input.symbol.upper()}
        )
...
async def get_account_balance() -> Dict[str, Any]:
    ...
    try:
        result = await make_binance_request(
            "/fapi/v2/account", signed=True, use_cache=False  # Don't cache account data
        )
        ...
        if result["success"]:
            ...
            return {
                ...
                - available_balance,  # Calculate used margin
...
async def set_leverage(input: SetLeverageInput) -> Dict[str, Any]:
    ...
    try:
        result = await make_binance_request(
            "/fapi/v1/leverage",
            method="POST",
            params={"symbol": input.symbol.upper(), "leverage": str(input.leverage)},
            signed=True,
            use_cache=False,
        )
        ...
        return {"success": False, "error": f"Failed to set leverage: {str(e)}"}
...
async def calculate_technical_indicators(
    ...
    try:
        ...
        return {"success": False, "error": f"Failed to calculate indicators: {str(e)}"}
...
async def get_server_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": f"Failed to get server status: {str(e)}"}


class SupportResistanceInput(BaseModel):
    symbol: str = Field(description="Trading pair symbol (e.g., BTCUSDT)")
    timeframe: str = Field(
        default="1h", description="Timeframe for analysis (1m, 5m, 15m, 1h, 4h, 1d)"
    )
...
async def calculate_support_resistance_levels(
    ...
    try:
        ...
        if not kline_result["success"]:
            return {
                "success": False,
                "error": f"Failed to get kline data: {kline_result['error']}",
            }
...
class MarketCorrelationInput(BaseModel):
    ...
    reference_symbols: List[str] = Field(
        ...
        description="Reference symbols for correlation analysis",
    )
...
async def analyze_market_correlation(input: MarketCorrelationInput) -> Dict[str, Any]:
    ...
    try:
        ...
        if not ticker_result["success"]:
            return {
                "success": False,
                "error": f"Failed to get ticker data: {ticker_result['error']}",
            }

        tickers = ticker_result["data"]
        ticker_dict = {
            ticker["symbol"]: float(ticker["priceChangePercent"]) for ticker in tickers
        }
        ...
        for ref_symbol in input.reference_symbols:
            if ref_symbol in ticker_dict:
                ...
                if (symbol_change > 0 and ref_change > 0) or (
                    symbol_change < 0 and ref_change < 0
                ):
                    correlation = (
                        "HIGH" if abs(symbol_change - ref_change) < 2 else "MEDIUM"
                    )
                else:
                    correlation = "LOW"
...
async def assess_market_sentiment(input: MarketSentimentInput) -> Dict[str, Any]:
    ...
    try:
        ...
        funding_result = await make_binance_request(
            "/fapi/v1/fundingRate", params={"symbol": input.symbol.upper(), "limit": 1}
        )

        open_interest_result = await make_binance_request(
            "/fapi/v1/openInterest", params={"symbol": input.symbol.upper()}
        )
        ...
        current_funding = (
            float(funding_rate.get("fundingRate", 0.0)) if funding_rate else 0.0
        )
        oi_value = (
            float(open_interest.get("openInterest", 0.0)) if open_interest else 0.0
        )
    ...
    try:
        ...
        for tf in input.timeframes:
            try:
                ...
                if not kline_result["success"]:
                    multi_tf_data[tf] = {
                        "error": f"Failed to get data: {kline_result['error']}"
                    }
                    continue
        ...
        trends = [
            ...
            for data in multi_tf_data.values()
        ...
        return {"success": False, "error": f"Multi-timeframe analysis failed: {str(e)}"}


if __name__ == "__main__":
    logger.info("🚀 Starting Binance FastMCP Server...")
    logger.info(
        "✅ All Binance trading functionality integrated with standards-compliant MCP protocol"
    )
    mcp.run()
