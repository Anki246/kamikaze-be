...
from infrastructure.database_config import DatabaseConfig, SCHEMA_DEFINITIONS, INDEX_DEFINITIONS
...
class DatabaseMigrator:
    ...
    async def connect_databases(self) -> bool:
        ...
        try:
            ...
            self.source_pool = await asyncpg.create_pool(**source_config.connection_params)
            ...
            if not all([target_config.host, target_config.user, target_config.password]):
                raise ValueError("RDS credentials not provided. Set DB_HOST, DB_USER, DB_PASSWORD environment variables.")
            
            self.target_pool = await asyncpg.create_pool(**target_config.connection_params)
    ...
    async def migrate_table_data(self, table_name: str) -> bool:
        ...
        try:
            ...
            async with self.source_pool.acquire() as source_conn:
                ...
                # Get all data
                rows = await source_conn.fetch(f"SELECT {column_list} FROM {table_name}")
            ...
            async with self.target_pool.acquire() as target_conn:
                # Clear existing data
                await target_conn.execute(f"TRUNCATE TABLE {table_name} RESTART IDENTITY CASCADE")
    ...
    async def reset_sequences(self, conn: asyncpg.Connection, table_name: str):
        ...
        try:
            ...
            for seq in sequences:
                ...
                max_val = await conn.fetchval(f"SELECT COALESCE(MAX({column_name}), 0) FROM {table_name}")
