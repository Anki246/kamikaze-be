...
class SystemHealthMonitor:
    ...
    def check_system_resources(self):
        ...
        memory_status = "✅" if memory_percent < 80 else "⚠️" if memory_percent < 95 else "❌"
        print(f"{memory_status} Memory Usage: {memory_percent:.1f}% ({memory.used / 1024**3:.1f}GB / {memory.total / 1024**3:.1f}GB)")
        ...
        try:
            ...
            connections = 0
            conn_status = "⚠️"
            print(f"{conn_status} Network Connections: Permission denied (requires elevated privileges)")
    ...
    def check_log_health(self):
        ...
        size_status = "✅" if total_size_mb < 100 else "⚠️" if total_size_mb < 500 else "❌"
    ...
    def generate_health_report(self, system_resources, log_health, config_health, mcp_health):
        ...
        if system_resources['cpu']['percent'] > 80:
            recommendations.append("High CPU usage - consider reducing trading frequency")
...
Examples:
  ...
  python utils/system_health.py --report         # Detailed health report
    ...
    parser.add_argument('--check', action='store_true', help='Perform quick health check')
    ...
    if args.check or args.report:
        ...
        if args.report:
            monitor.generate_health_report(system_resources, log_health, config_health, mcp_health)
